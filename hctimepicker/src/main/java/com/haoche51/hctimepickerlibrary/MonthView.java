...
public class MonthView extends LinearLayout {
	...
	public static MonthView create(ViewGroup parent, LayoutInflater inflater,
		...
		for (int offset = 0; offset < 7; offset++) {
			...
			textView.setText(weekDays[today.get(Calendar.DAY_OF_WEEK) - 1]);
//			textView.setText(weekdayNameFormat.format(today.getTime()));//周一...格式
		}
		...
		for (int i = 0; i < 6; i++) {
			...
			if (i < numRows) {
				...
				for (int c = 0; c < week.size(); c++) {
					...
					//不是本月日期设置不可见
					cellView.setVisibility(cell.isCurrentMonth() ? VISIBLE : GONE);
