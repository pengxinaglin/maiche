...
public class BuyerRecommendFragment extends CommonBaseFragment implements HCPullToRefresh.OnRefreshCallback, View.OnClickListener {
	...
	public static final int REQUEST_CODE_ADD_CLUES = 100;
	...
	protected void initData(Bundle savedInstanceState) {
		...
		if (!TextUtils.isEmpty(json)) {
			this.allLeads.clear();
			this.allLeads.addAll(HCJsonParse.parseGetBuyerCluesEntitys(json));
		}
		if (mAdapter == null) {
	...
	public void onHttpComplete(String action, int requestId, HCHttpResponse response, Throwable error) {
		...
		if (HttpConstants.ACTION_GET_BUYERLEAD_LIST.equals(action)) {
			dismissLoadingView();
		...
		switch (response.getErrno()) {
			case 0://请求成功
				try {
					...
					List<BuyerCluesEntity> tempList = HCJsonParse.parseBuyerCluesResult(obj.getString("list"));
	...
	public void onClick(View view) {
		if (view.getId() == R.id.floatingactionbutton) {
			//添加推荐买家
			Intent intent = new Intent(this.getActivity(), BuyerAddCluesActivity.class);
	...
	public void onPullDownRefresh() {
		pageIndex = 0;
		AppHttpServer.getInstance().post(HCHttpRequestParam.getBuyerleadList(pageIndex,
