package com.haoche51.sales.custom.tagtext;
...
public class TagTextView extends TextView {
	...
	public void render() {
		...
		if (needWidth > hasWidth) {
			// 进行截取截取
			float oneWidth = textPaint.measureText("直");
	...
	private void applyRenderers(View tagView) {
		if (mViewSpanRender != null) {
			Spannable spannableString = new SpannableString(getText().toString().substring(0, 1) + getText());
			if (getText().toString().contains(mFlagChar)) {
				...
				spannableString.setSpan(new TagImageSpan(bitmapDrawable), 0, 1, DEFAULT_RENDER_APPLY_MODE);
			}
			setText(spannableString.subSequence(0, mStart));
		}
	}

	public interface ViewSpanRender {
		View getView(final String text, final Context context, float width);
