...
/**
 * 看车失败列表
 * Created by PengXianglin on 16/11/25.
 */
public class TransFailedFragment extends CommonBaseFragment implements AdapterView.OnItemClickListener, View.OnClickListener, HCPullToRefresh.OnRefreshCallback {
	...
	protected int getContentView() {
		return R.layout.fragment_trans_failed;
	}
	...
	protected void initData(Bundle savedInstanceState) {
		if (this.fitlerEntity == null) {
			...
			this.fitlerEntity.setType(TaskConstants.TRANSACTION_FAILED);//0待看车1失败 2成功
		}
		//初始化ACache
		if (mAcache == null) {
		...
		//读取ACache中的缓存记录
		String json = mAcache.getAsString(TaskConstants.ACACHE_TRANSACTION_FAILED);
	...
	public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
		...
		map.put("trans_id", mTasks.get(position).getId());
		HCActionUtil.launchActivity(this.getActivity(), TransFailedDetailActivity.class, map);
	}
			...
			case 0://请求成功
				...
				if (requestId == HttpConstants.GET_LIST_REFRESH && viewTaskList != null) { //刷新
					if (!TextUtils.isEmpty(response.getData())) {
						mAcache.put(TaskConstants.ACACHE_TRANSACTION_FAILED, response.getData());//只缓存刷新的数据
					}
					mTasks.clear();
